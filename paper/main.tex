...
Before modeling, we removed clearly implausible split entries (conservative lower-bound timing artifacts; e.g., single-digit station times) as likely recording errors.
Beyond that quality-control filter, we did not perform broad outlier deletion because extreme yet plausible splits are part of the target population; instead, we used robust summaries (medians, quantile regression) and report a winsorization-based sensitivity check for SHAP rankings in Section~\ref{sec:limitations}.
...
This approach is immune to the compositional $R^2$ inflation because it examines \emph{relative} coefficient magnitudes across quantiles rather than absolute prediction accuracy.
...
The low absolute silhouette score is consistent with the continuous gradient structure and indicates weak cluster separation.
Accordingly, we interpret the $K=2$ solution as a pragmatic binning of a continuous spectrum, not evidence of naturally discrete athlete types.
...
Notably, the top-3 features all occur in the \emph{final quarter} of the race, suggesting that cumulative fatigue resistance is the dominant source of inter-individual variance.
...
For within-event percentile rank, XGBoost achieved $R^2=0.663 \pm 0.064$ (MAE = 0.124, RMSE = 0.167) and Elastic Net achieved $R^2=0.634 \pm 0.062$ (MAE = 0.134, RMSE = 0.174).
For within-event residual finish time, XGBoost achieved $R^2=0.769 \pm 0.036$ (MAE = 6.56 min, RMSE = 8.54 min) and Elastic Net achieved $R^2=0.675 \pm 0.047$ (MAE = 7.25 min, RMSE = 10.11 min).
...
For percentile rank, XGBoost achieved $R^2=0.640 \pm 0.060$ (MAE = 0.131, RMSE = 0.173) and Elastic Net achieved $R^2=0.622 \pm 0.045$ (MAE = 0.138, RMSE = 0.177).
For residual finish time, XGBoost achieved $R^2=0.744 \pm 0.037$ (MAE = 6.83 min, RMSE = 8.99 min) and Elastic Net achieved $R^2=0.690 \pm 0.038$ (MAE = 7.10 min, RMSE = 9.89 min).
...
In a supplementary fixed-effect check on total time (pooled random 5-fold; descriptive rather than external), adding event dummies improved performance (Ridge: $R^2$ from 0.834 to 0.859; MAE from 5.74 to 5.21 min), consistent with measurable between-event condition effects.

\subsection{Pacing Collapse Accelerates Beyond 90 Minutes}
...
The critical observation is the \emph{nonlinear jump} between Sub-80 (22\%) and Sub-100 (37\%).
...
This threshold at $\sim$90 minutes aligns with the known glycogen depletion onset in prolonged exercise~\cite{coyle1986glycogen}, a phenomenon well-documented in marathon pacing research~\cite{abbiss2008pacing}.
Our finding extends this threshold to hybrid-format events, where interleaved high-intensity workouts likely accelerate glycogen utilization compared to steady-state running.
...
This quantile-dependent structure has no analog in the existing CrossFit or endurance sports literature and represents a novel contribution.
...
This pattern is consistent with Japan's fitness culture emphasizing running over resistance training~\cite{sasakawa2020fitness}.
...
The practical implication is that \emph{shortcuts do not exist}: compensating weak running with strong workouts is not viable at the population level.
...
Multiple independent analyses converge to form our central theoretical contribution---the \textbf{Late-Race Hypothesis}:
\begin{quote}
\emph{The dominant source of inter-individual variance in HYROX finish time is the ability to maintain performance under cumulative fatigue in the final race quarter (Wall Balls, Run~7--8).}
\end{quote}

Evidence for this hypothesis comes from three independent lines:
...
(2)~Japan's negative $d$ for Run~1 ($-$0.82) and Run~8 ($-$0.25) suggests strong pure running ability, consistent with Japan's deep running culture~\cite{sasakawa2020fitness}.
...
\textbf{Outlier sensitivity}: We removed clearly implausible timing artifacts during preprocessing, but did not broadly trim plausible extreme performances, because tail behavior is intrinsic to real race populations.
In a winsorization sensitivity check (1st--99th percentile clipping; $n=20{,}000$ subsample), SHAP rank stability remained high (Spearman $\rho=0.985$; top-5 overlap = 5/5), supporting robustness of the primary hierarchy.
...
By combining eight complementary analytical frameworks, we identify five principal findings:

\begin{enumerate}[nosep]
  \item HYROX performance is fundamentally unidimensional (PC1 = 57.4\%), governed by a ``general fitness'' factor.
  \item Late-race performance is the dominant source of inter-individual variance: Wall Balls (SHAP = 2.28~min [2.15, 2.41]) and Run~8 (2.14 [1.75, 2.50]) are consistently prominent, while Run~7 shows wider uncertainty (2.05 [1.20, 2.90]).
  \item Pacing collapse follows an exponential pattern with a threshold at $\sim$90 minutes.
...
These findings provide a data-driven foundation for HYROX training design and race-day strategy.
